...
// --- CUSTOM MODAL COMPONENT (Fixed Centering) ---
    ...
    return (
        <div style={{ 
            ...
            >
                ...
                <p style={{ margin: '0 0 25px 0', color: '#6b7280', fontSize: '14px', lineHeight: '1.5', whiteSpace: 'pre-line' }}>
                    {message}
                </p>
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                    <button 
                        onClick={onClose} 
                        style={{ padding: '10px 20px', borderRadius: '10px', border: '1px solid #e5e7eb', background: 'white', cursor: 'pointer', fontWeight: '600', color: '#374151' }}
                    >
                        Cancel
                    </button>
                    <button 
                        ...
                        style={{ 
                            ...
                            boxShadow: isDanger ? '0 4px 14px 0 rgba(220, 38, 38, 0.39)' : '0 4px 14px 0 rgba(37, 99, 235, 0.39)'
                        }}
                    >
...
export default function SuperAdminDashboard() {
    ...
    const handleDeleteInstitute = (app) => {
        openConfirm("Delete Institute?", `⚠️ This will PERMANENTLY DELETE "${app.instituteName}" and all its data (Users, Attendance, etc).`, async () => {
            closeConfirm();
            ...
            try {
                const response = await fetch(`${BACKEND_URL}/deleteInstitute`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ instituteId: app.id })
                });
    ...
    const handleSendLoginLink = async (email) => {
        ...
        try { 
            ...
            toast.success(`Link sent to ${email}`, { id: toastId });
        ...
        <div className="dashboard-container">
            ...
            <main className="main-content">
                ...
                <div className="content-section">
                    ...
                    {/* SECTION 1: PENDING APPLICATIONS (With Document Link) */}
                    ...
                    <div className="card card-full-width">
                        <div className="table-wrapper">
                            <table className="attendance-table">
                                <thead>
                                    <tr><th>Institute</th><th>Contact</th><th>Email</th><th>Document</th><th>Actions</th></tr>
                                </thead>
                                <tbody>
                    ...
                    {/* SECTION 2: APPROVED INSTITUTES (With Document Link) */}
                    ...
                    <div className="card card-full-width">
                        <div className="table-wrapper">
                            <table className="attendance-table">
                                <thead>
                                    <tr><th>Institute</th><th>Admin Contact</th><th>Email</th><th>Document</th><th>Actions</th></tr>
                                </thead>
                                <tbody>
                                    {approvedApps.length > 0 ? approvedApps.map(app => (
                                        <tr key={app.id}>
                                            ...
                                            {/* ✅ VIEW DOCUMENT LINK (Also in Approved) */}
                                            <td>
                                                {app.documentUrl ? (
                                                    <a 
                                                        href={app.documentUrl} 
                                            ...
                                            <td>
                                                <div style={{display:'flex', gap:'8px'}}>
                                                    <button onClick={() => handleSendLoginLink(app.email)} className="btn-action btn-action-link" title="Resend Login Link">
                                                        <i className="fas fa-paper-plane"></i>
                                                    </button>
                                                    <button onClick={() => handleDeleteInstitute(app)} className="btn-action btn-action-deny" style={{backgroundColor: '#fee2e2', color: '#dc2626', border:'1px solid #fecaca'}} title="Delete Institute">
